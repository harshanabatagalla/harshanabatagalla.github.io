        ...
        <div className="h-full p-6 overflow-auto font-[JetBrains_Mono]">
            ...
            <div className="space-y-8">
                {educationData.map((education, index) => (
                    <motion.div
                        key={education.id}
                        className="bg-[#1f2428] rounded-lg p-6 border-l-4 border-[#f9826c] hover:border-l-8 hover:shadow-lg opacity-0"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {certifications.map((certification, index) => (
                    <motion.div
                        ...
                        className="bg-[#1f2428] p-4 rounded-lg border border-[#1f2428] cursor-pointer hover:bg-[#414246] transition-colors duration-300 opacity-0"
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.6, delay: index * 0.1, ease: "easeOut" }}
                        viewport={{ once: true }}
